...
s7b <- sim.pops(n.sims=n.sims, x=s7b.x, theta=thetas)
s7c <- sim.pops(n.sims=n.sims, x=s7c.x, theta=thetas)
s7d <- sim.pops(n.sims=n.sims, x=s7d.x, theta=thetas)
...
s7c_coefs<-est_coefs(s7c, formula="recruits~n + z + z2 + n:z", years=c((n.years-50):n.years))
s7d_coefs<-est_coefs(s7d, formula="recruits~n + z + z2 + n:z", years=c((n.years-50):n.years))

s7b_reg_eqs<-additiveFD_eq(s7b_coefs)
s7c_reg_eqs<-DDS_eq(s7c_coefs)
s7d_reg_eqs<-DDS_eq(s7d_coefs)
...
s7_eq_reg_z<-tapply(s7_reg_eqs$eq_z, s7_reg_eqs$scen, median)
