...
sim_values=c(s5c.x$BZn, s5b.x$BZn, s5d.x$BZn)
...
Bz  <- -theta[2] * 2 * Bq
theta_pred<-rep(round(Bz/-(2*Bq),2), length(BZn))
...
##Plot
plot_land()
mtext("E) FD Dens reg", side = 3, cex=0.8)
...
points(s5_eq_sim_n~s5_eq_sim_z,  col="black", cex=2, bg=alpha(cols,0.2), pch=21)
